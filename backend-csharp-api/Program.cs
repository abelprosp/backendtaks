...
{
    options.Limits.MaxRequestBodySize = 52_428_800; // 50 MB (anexos em base64)
});
...
builder.Services.Configure<AppOptions>(options =>
{
    ...
    options.LegacyPassword = builder.Configuration["LEGACY_PASSWORD"] ?? string.Empty;
    options.PreferLegacyAttachments = bool.TryParse(builder.Configuration["PREFER_LEGACY_ATTACHMENTS"], out var preferLegacyAttachments) && preferLegacyAttachments;
    options.RequireLegacyAttachments = bool.TryParse(builder.Configuration["REQUIRE_LEGACY_ATTACHMENTS"], out var requireLegacyAttachments) && requireLegacyAttachments;
