-- Align prazo condition with the frontend label "Vencido / vence hoje":
-- active demands due today belong to "vencido", not "no_prazo".
  ...
  filtered as (
    ...
    where (
      ...
      and (
        ...
        or (p_condicao_prazo = 'vencido' and d.status::text in ('em_aberto', 'em_andamento') and d.prazo is not null and d.prazo <= (now() at time zone 'America/Sao_Paulo')::date)
        or (p_condicao_prazo = 'no_prazo' and d.status::text in ('em_aberto', 'em_andamento') and d.prazo is not null and d.prazo > (now() at time zone 'America/Sao_Paulo')::date)
      )
