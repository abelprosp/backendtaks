-- RPCs para consolidar listagem paginada e detalhe protegido de demandas.
...
as $$
  ...
  filtered as (
    ...
    where (p_ids is null or d.id = any(p_ids))
...
create or replace function public.rpc_demanda_detail_for_user(
  p_user_id uuid,
  p_demanda_id uuid
)
returns table (
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'ordem', st.ordem,
          'responsavelUserId', st.responsavel_user_id,
          'responsavel',
            case
        ...
        order by st.ordem asc, st.id asc
      )
      from public.subtarefa st
      left join public."User" u on u.id = st.responsavel_user_id
      where st.demanda_id = d.id
    ), '[]'::jsonb) as subtarefas,
    coalesce((
      select jsonb_agg(
